...
const routes = {
  'POST /api/signup': async (req, res) => {
    ...
    const user = db.createUser({ name, email, passwordHash: hash, passwordSalt: salt, role, avatarUrl, preferredLanguage });
...
function userProfileRoute(req, res, id) {
  ...
  send(res, 200, { profile: publicProfile(target) });
}
...
wss.on('connection', (conn, req) => {
  ...
  conn.on('close', () => {
    ...
    if (set) {
      set.delete(conn);
      if (!set.size) userConns.delete(authedUser.id);
    }
